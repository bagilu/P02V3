import { addQuestion, closeDiscussion, getQuestions, getTeacherState, setActiveQuestion, submitFacilitatorIdea } from './api.js';
...
function updateStateUi(state){
  ...
  if(hasActive) facilitatorIdeaQuestion.innerHTML=`<strong>Q${state.active_question_sort_order || ''}</strong> ${escapeHtml(state.active_question_text||'')}`;
}
...
btnSubmitFacilitatorIdea?.addEventListener('click',async()=>{clearMessage(facilitatorIdeaMessage);const text=(facilitatorIdeaText.value||'').trim();if(!currentState?.active_question_id){return setMessage(facilitatorIdeaMessage,'目前沒有作用中的問題。','warning');}if(!text)return setMessage(facilitatorIdeaMessage,'請先輸入想法。 / Enter an idea first.','warning');btnSubmitFacilitatorIdea.disabled=true;try{await submitFacilitatorIdea(discussionId,currentState.active_question_id,teacherToken,text);setMessage(facilitatorIdeaMessage,'想法已保存；再次送出會更新這一則想法。 / Idea saved; submit again to update it.','success');}catch(e){setMessage(facilitatorIdeaMessage,e.message||'送出失敗。','danger');}finally{btnSubmitFacilitatorIdea.disabled=false;}});
